...
4. **조립** — 결과를 아래 출력 템플릿에 맞춰 Markdown으로 짠다. 표는 Cypher 결과를 그대로,
   흐름은 Mermaid로 변환한다(변환법은 recipes 문서 끝).
...
- 커맨드별 표: `커맨드 | 인터페이스 메서드 | 서버 구현 | 종착 엔티티`
...
## 소스 코드 보강 (선택, 현재 그래프엔 코드 본문이 없음)

현 그래프는 **구조만** 담고 소스 본문·시맨틱은 없다(Phase 2 예정). 사용자가 코드 스니펫이나
"무슨 일을 하는지" 설명을 원하면, 그래프로 **무엇을/어디를** 찾은 뒤 Claude의 `Read`/`Grep`/
`Glob`로 Vanuatu 소스를 직접 열어 보강한다.
...
- 그래프의 `fullName`(예: `Torba.Service.Order.OrderService.SearchOrdersAsync`)으로 타입/메서드명을
  `Grep`해 파일을 찾고, 해당 부분만 `Read`해 인용한다. 전체 파일 덤프는 피하고 관련 메서드만.
- 보강한 스니펫은 출처(파일 경로:라인)를 함께 적는다. 이 단계는 v2에서 그래프 자체에 본문이
  들어오면 단순해진다 — 그 전까지의 임시 보강이다.
...
- 저장소 [docs/cookbook.md](../../../docs/cookbook.md) — SQL 대조 학습 + 더 많은 레시피·워크드 예제.
